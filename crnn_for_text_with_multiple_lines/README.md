...
#### 改进方向
+ 目前特征提取直接使用Resnet将原图下采样到(H<sub>i</sub>/16, W<sub>i</sub>/16)，再将特征Rearrange("B C H W->B (H W) C")后送入Transformer，而标准CRNN是将原图下采样到(H<sub>i</sub>/32, W<sub>i</sub>/4)，这样可能会导致小号字符难以学习，训练过程中也发现小号字符难以训练
