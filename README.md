...
-----------------------------------------
#### 端到端文档理解 Document Understanding 
...
+ 会不可避免地丢失图片中的一些信息，例如图片中插图、背景图、字体、线框、色彩等等，这些丢失信息对理解图片内容可能是不可缺少的
+ OCR本身识别错误影响后续文档理解
+ 存在表格、弯曲文本、图表、公式、印章等等OCR难以识别的内容

因此，我认为端到端文档理解是更可行的技术方案。简单来说，就是需要将图片编码成一维序列，使得TransformerDecoder可以像处理文本一样处理图片。
